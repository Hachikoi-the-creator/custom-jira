    ...
    <AppLayout>
      <div className="max-w-4xl">
        <div className="flex items-center justify-between mb-6">
          <div><h1 className="page-title">Base de problemas</h1><p className="text-slate-500 text-sm mt-1">{problems.length} problemas — busca para encontrar soluciones pasadas</p></div>
        ...
        {query.trim() && (
          <div className="mb-6">
            <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-3">{results.length} resultado{results.length !== 1 ? 's' : ''} para "{query}"</p>
            {results.length > 0 ? (
              <div className="space-y-3">
                {results.map((p: any) => (
                  <Link key={p.id} href={`/problems/${p.id}`} className="card block hover:border-amber-300 hover:shadow-md transition-all">
                    ...
                    {p.client && <p className="text-xs text-slate-400 mt-2">{p.client.name}</p>}
                  </Link>
                ))}
              </div>
            ) : (
              <div className="card text-center py-8">
                <p className="text-slate-500 text-sm">No se encontraron problemas similares</p>
        ...
        {!query.trim() && (
          ...
          : !problems.length ? (
            <div className="card text-center py-16">
              ...
              <p className="text-slate-500 font-medium">Sin problemas registrados</p>
                ...
                <Link key={p.id} href={`/problems/${p.id}`} className="card hover:shadow-md transition-all hover:border-amber-200 flex items-center justify-between group">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-slate-800 group-hover:text-amber-700">{p.title}</p>
                      <p className="text-sm text-slate-500 mt-0.5 line-clamp-1">{p.description}</p>
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        {p.client && <span className="text-xs text-slate-400">{p.client.name}</span>}
