...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: 'Clientes', value: clientsCount ?? 0, icon: Users, href: '/clients', color: 'bg-primary/10 text-primary' },
    ...
    <AppLayout>
      <div className="max-w-5xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">{format(new Date(), "EEEE d 'de' MMMM, yyyy", { locale: es })}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><CheckSquare size={16} className="text-muted-foreground" /><h2 className="font-semibold text-foreground">Pendientes</h2></div>
              <Link href="/meetings" className="text-xs text-primary hover:underline">Ver todo</Link>
            </div>
            {!pendingItems?.length ? (
              <p className="text-muted-foreground text-sm py-4 text-center">Sin pendientes 🎉</p>
            ) : (
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><Clock size={16} className="text-muted-foreground" /><h2 className="font-semibold text-foreground">Reuniones recientes</h2></div>
              <Link href="/meetings" className="text-xs text-primary hover:underline">Ver todo</Link>
            </div>
            {!recentMeetings?.length ? (
              <p className="text-muted-foreground text-sm py-4 text-center">Sin reuniones aún</p>
            ) : (
                ...
                {recentMeetings.map((m: any) => (
                  <li key={m.id}>
                    <Link href={`/meetings/${m.id}`} className="block hover:bg-muted -mx-2 px-2 py-1.5 rounded-lg transition-colors">
                      ...
                      <p className="text-xs text-muted-foreground">{m.client?.name ?? 'Sin cliente'} · {format(new Date(m.date), 'd MMM yyyy', { locale: es })}</p>
                    </Link>
                  </li>
                ))}
              </ul>
