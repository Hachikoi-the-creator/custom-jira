    ...
    <AppLayout>
      <div className="max-w-4xl">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="page-title">Meetings</h1><p className="text-muted-foreground text-sm mt-1">{meetings?.length ?? 0} meetings</p></div>
          <Link href="/meetings/new" className="btn-primary flex items-center gap-2"><Plus size={16} />New meeting</Link>
        </div>
        {!meetings?.length ? (
          <div className="card text-center py-16">
            <CalendarDays size={40} className="text-muted-foreground/40 mx-auto mb-3" />
            <p className="text-muted-foreground font-medium">No meetings</p>
            <Link href="/meetings/new" className="btn-primary inline-flex items-center gap-2 mt-4"><Plus size={16} />New meeting</Link>
          </div>
        ) : (
          <div className="space-y-2">
            {meetings.map((m: any) => (
              <Link key={m.id} href={`/meetings/${m.id}`} className="card hover:shadow-md transition-all hover:border-primary/30 flex items-center justify-between group">
                ...
                <p className="text-sm text-muted-foreground">{format(new Date(m.date), 'd MMM yyyy', { locale: enUS })}</p>
              </Link>
            ))}
          </div>
        )}
